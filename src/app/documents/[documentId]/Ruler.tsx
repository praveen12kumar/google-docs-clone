                ...
                <div className="relative h-full w-[816px]">
                    {
                        markers.map((marker)=>{
                            ...
                            return(
                                <div className="absolute bottom-0" key={marker}
                                    style={{left: `${position}px`}}>
                                        {
                                            marker % 10 == 0 && (
                                                <>
                                                    <div className="absolute bottom-0 w-[1px] h-2 bg-neutral-500"/>
                                                    <span className='absolute bottom-2  text-[10px] text-neutral-500 transform -tansform-x-1/2'>
                                                        {marker /10 + 1}
                                                    </span>
...
const Marker = ({position, isLeft, isDragging, onMouseDown, onDoubleClick}: MarkerProps) => {
    ...
    >
        <FaCaretDown className='absolute top-0 left-1/2 h-full fill-blue-600 transform -translate-x-1/2'/>
        <div className="absolute left-1/2 top-4 transform -translate-x-1/2 translate-opacity duration-150"
            style={{
                height:"100vh",
                width: "1px",
                transform: "scaleX(0.5)",
                backgroundColor: '#3b72f5',
